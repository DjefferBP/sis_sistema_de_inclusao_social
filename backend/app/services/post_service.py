...
class PostService:
    ...
    async def listar_posts_usuario(self, user_id: int, limit: int = 20, offset: int = 0) -> Dict[str, Any]:
    ...
    async def listar_posts_por_categoria(self, categoria: str, limit: int = 20, offset: int = 0) -> Dict[str, Any]:
    ...
    async def descurtir_post(self, post_id: int, user_id: int) -> Dict[str, Any]:
        ...
        if not resultado_curtidas:
            raise HTTPException(
                401, 'Não foi possível descurtir o post!'
            )
